...
```{r}
df <- data_filt %>% group_by(cogload) %>% summarize(sdrt = sd(reaction_prompt_millis), meanrt = mean(reaction_prompt_millis), sdperf = sd(perf, na.rm = TRUE), meanperf = mean(perf, na.rm=TRUE))
...
```{r}
df <- data_filt %>% group_by(reward) %>% summarize(sdrt = sd(reaction_prompt_millis), meanrt = mean(reaction_prompt_millis), sdperf = sd(perf, na.rm = TRUE), meanperf = mean(perf, na.rm=TRUE))
